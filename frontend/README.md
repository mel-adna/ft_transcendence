...
Both rough edges that used to sit here are fixed on the backend. Signing in before verifying now answers `403 Account is disabled. Please verify your email first.` instead of a 500, so `getErrorMessage` shows something useful. And `POST /auth/resend-verification` exists, which rotates the stored code and emails a new one. The verify screen still does not offer a resend button; adding one is a small piece of frontend work now that there is an endpoint behind it.
